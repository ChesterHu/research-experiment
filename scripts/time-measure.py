...
DatasetName = "ca-GrQc-cc"

ref_node = [3]
alphas = np.linspace(0.1, 0.5, 50)
rhos = np.logspace(-4, -7, 50)
epsilons = np.logspace(-2, -4, 50)

print("loading graph...")
g = lgc.GraphLocal("../../LocalGraphClustering/notebooks/datasets/{0}.graphml".format(DatasetName), "graphml")
...
if __name__ == "__main__":
    plotTime("../figures/run-time-eps-{}-seed{}.png".format(DatasetName, ref_node[0][0]), measureTimeEpsilon, epsilons, "epsilon")
    plotTime("../figures/run-time-alpha-{}-seed{}.png".format(DatasetName, ref_node[0][0]), measureTimeAlpha, alphas, "alpha")
    plotTime("../figures/run-time-rho-{}-seed{}.png".format(DatasetName, ref_node[0][0]), measureTimeRho, rhos, "rho")
