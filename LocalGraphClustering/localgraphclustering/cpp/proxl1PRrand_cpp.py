...
_graphlib_funs_proxl1PRaccel_rand64 = _setup_proxl1PRrand_args(
    'int64','int64', _graphlib.proxl1PRaccel_rand64)
_graphlib_funs_proxl1PRaccel_rand32 = _setup_proxl1PRrand_args(
    'uint32','uint32', _graphlib.proxl1PRaccel_rand32)
_graphlib_funs_proxl1PRaccel_rand32_64 = _setup_proxl1PRrand_args(
    'uint32','int64', _graphlib.proxl1PRaccel_rand32_64)

def _get_proxl1PRrand_cpp_types_fun(ai,aj, accel = False):
    float_type,vtype,itype,ctypes_vtype,ctypes_itype = determine_types(ai,aj)
    if (vtype, itype) == (np.int64, np.int64):
        fun = _graphlib_funs_proxl1PRrand64 if not accel else _graphlib_funs_proxl1PRaccel_rand64
    elif (vtype, itype) == (np.uint32, np.int64):
        fun = _graphlib_funs_proxl1PRrand32_64 if not accel else _graphlib_funs_proxl1PRaccel_rand32_64
    else:
        fun = _graphlib_funs_proxl1PRrand32 if not accel else _graphlib_funs_proxl1PRaccel_rand32
...
def proxl1PRrand_cpp(ai,aj,a,ref_node,d,ds,dsinv, y=None,alpha = 0.15,rho = 1.0e-5,epsilon = 1.0e-4,maxiter = 10000,max_time = 100,normalized_objective=True):
    ...
    return compute(ai,aj,a,ref_node,d,ds,dsinv, float_type,vtype,itype,ctypes_vtype,ctypes_itype,fun, y=y,alpha = alpha,rho = rho,epsilon = epsilon,maxiter = maxiter,max_time = max_time,normalized_objective=normalized_objective)

def proxl1PRaccel_rand_cpp(ai,aj,a,ref_node,d,ds,dsinv, y=None,alpha = 0.15,rho = 1.0e-5,epsilon = 1.0e-4,maxiter = 10000,max_time = 100,normalized_objective=True):
    float_type,vtype,itype,ctypes_vtype,ctypes_itype,fun = _get_proxl1PRrand_cpp_types_fun(ai,aj, accel = True)
    return compute(ai,aj,a,ref_node,d,ds,dsinv, float_type,vtype,itype,ctypes_vtype,ctypes_itype,fun, y=y,alpha = alpha,rho = rho,epsilon = epsilon,maxiter = maxiter,max_time = max_time,normalized_objective=normalized_objective)

def compute(ai,aj,a,ref_node,d,ds,dsinv, float_type,vtype,itype,ctypes_vtype,ctypes_itype,fun, y=None,alpha = 0.15,rho = 1.0e-5,epsilon = 1.0e-4,maxiter = 10000,max_time = 100,normalized_objective=True):
    n = len(ai) - 1
